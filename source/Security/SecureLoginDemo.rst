...
* パスワード変更履歴エンティティの保存
  ...
  * 変更後のパスワードの使用開始日時
  ...
  .. note ::
     ...
     しかしながら、:ref:`パスワードの品質チェック <password-strength>` の要件の実現に使用するため、パスワード変更履歴エンティティに含めて保存する。
...
.. tip ::
   ...
   本アプリケーションでは、テーブルをシンプルな状態に保ち、エントリの不要な更新を避けて単純に挿入と削除を使用することで要件を実現するために、イベントエンティティを用いる実装方法を採用している。
