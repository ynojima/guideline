...
HTMLの入力フォームを使う方式やRFCで定められているHTTP標準の認証方式(Basic認証やDigest認証など)を利用のが一般的だが、
...
.. list-table:: **PasswordEncoderの実装クラス**
    ...
    * - | \ ``BCryptPasswordEncoder``\
      - | BCryptアルゴリズムを使用してパスワードのハッシュ化及び照合を行う実装クラス。
        ...
        | 詳細は、\ `BCryptPasswordEncoderのJavaDoc <http://docs.spring.io/spring-security/site/docs/4.0.3.RELEASE/apidocs/org/springframework/security/crypto/password/BCryptPasswordEncoder.html>`_\ を参照されたい。
...
.. list-table::
    ...
    * - | (2)
      - | コンストラクタの引数に、ハッシュ化のラウンド（ストレッチング）回数を指定する。
...
* bean定義ファイルに記述したパス(\ ``<form-login>``\ 要素の\ ``default-target-url``\ 属性に指定したパス)
...
\ ``RedirectAuthenticationHandler``\ をSpring Securityの認証処理に適用する。
...
.. list-table::
   ...
   * - 項番
     - 説明
   * - | (1)
     - | \ ``authentication-failure-handler-ref``\ (認証失敗時のハンドラ設定)と\ ``authentication-success-handler-ref``\ (認証成功時のハンドラ設定)のbeanを指定する。
   * - | (2)
   ...
   * - | (4)
     - | 認証失敗時の遷移先パスを指定する。
       | 上記例では、ログイン画面のパスと認証失敗後の遷移であることを示すクエリ(\ ``error=true``\ )を設定している。
   * - | (5)
     - | **本機能を使用する場合はuseForwardをtrueに指定する必要がある。**
       | \ ``true``\に指定することで、認証失敗時に表示する画面(ログイン画面)に遷移する際に、RedirectではなくForwardが使用される。
       | 
       | これは、認証処理を行うリクエストのリクエストパラメータの中に「認証成功後に遷移するページのURL」を含める必要があるためである。
       | Redirectを使用して認証エラー画面を表示してしまうと、「認証成功後に遷移するページのURL」がリクエストパラメータから引き継ぎことが出来ないため、認証が成功しても指定した画面に遷移することができない。
       | この事象を回避するためには、Forwardを使用して「認証成功後に遷移するページのURL」をリクエストパラメータから引き継げるようにしておく必要がある。
...
ログイン画面に、「認証成功後に遷移するページのURL」をリクエストパラメータとして連携する。
...
.. code-block:: jsp
    ...
    <form:form action="${pageContext.request.contextPath}/login" method="get">
        ...
        <input type="hidden"
               name="redirectTo"
               value="${pageContext.request.contextPath}/reservetour/read?${f:query(reserveTourForm)}
                      &page.page=${f:h(param['page.page'])}
...
認証処理に、遷移元の画面から渡された「認証成功後に遷移するページのURL」を連携する。
