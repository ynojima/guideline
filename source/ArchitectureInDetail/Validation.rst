...
日時フォーマットの単項目チェック
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
| 日時フォーマットのチェックを行う場合には、Bean Validationの仕組みではなく、Springが提供する日時の型変換を行うための\ ``@DateTimeFormat``\ アノテーションの使用を推奨する。
...
| \ ``@DateTimeFormat``\ アノテーションはSpringが提供する型変換の仕組みのひとつであるので、入力エラーの場合には、Bean Validationのエラーメッセージではなく、Springの型変換例外のメッセージがそのまま画面へバインドされる。
| デフォルトのメッセージが画面にバインドされることを避けるため、型変換例外のメッセージを設定する必要がある。
