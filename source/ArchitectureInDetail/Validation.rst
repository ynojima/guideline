...
日時フォーマットの単項目チェック
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
| 日時フォーマットのチェックを行う場合には、日時の型変換を行う\ ``@DateTimeFormat``\ アノテーションの使用を推奨する。
| \ ``@DateTimeFormat``\ アノテーションの使用方法については、\ :ref:`ApplicationLayer-DateTimeFormat`\ を参照されたい。
| Bean Validationの\ ``@Pattern``\ アノテーションを使用することでも日時フォーマットのチェックは可能であるが、日時フォーマットを正規表現で記述する必要があるため、\ ``@DateTimeFormat``\ アノテーションのほうが実装がシンプルになる。


| \ ``@DateTimeFormat``\ アノテーションを使用すると、入力エラーの場合は型の変換が不可能な値として処理され、例外の原因がそのままエラーメッセージとして表示されてしまう。
| そのため、エラーメッセージの定義を行う必要がある。
